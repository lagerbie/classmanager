...
/*
 * v0.95.10: modif de bytesToInt(byte[] bytes, int offset, int length, boolean bigEndian)
 *           [utilisation de | au lieu de +]
 * v0.95.12: modif de createWaveImages(...) [dessine ligne blanche si begin==end,
 *           sécurisation si valueMax==0]
 * v0.95.12: modif de draw(...) [add file.delete() et sécurisation si nPoint<=0]
 * v0.95.13: modif de draw(...) [add RenderingHints]
 * 
 * v0.96: modif de draw(...) [mibération ressources]
 */
...
public class Wave {
     ...
     */
    public static void createWaveImages(File leftChannelFile, File rigthChannelFile,
            AudioFormat format, ByteBuffer buffer, long begin, long end) {
...
//        valueMax = Math.pow(2, sampleSize * 8 -1);
    ...
    private static int bytesToInt(byte[] bytes, int offset, int length, boolean bigEndian) {
        ...
        return value;
    }//end bytesToInt(byte[] bytes, int offset, int length, boolean bigEndian)
...
//
//        return bytes;
//    }//end intToBytes(int integer, int length, boolean bigEndian)
