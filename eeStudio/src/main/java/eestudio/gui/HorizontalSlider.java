...
/*
 * v1.02: modif de HorizontalSlider(..) [modification des temps d'apparition des
 *        tooltip: InitialDelay 750ms à 200ms, et ReshowDelay 500ms à 200ms]
 * v1.02: modif de addListeners() [ajout de ToolTipManager.sharedInstance()
 *        .mouseMoved(event) dans mouseDragged(MouseEvent event) pour actualiser
 *        le tooltip sur le drag]
 */
...
//        ToolTipManager.sharedInstance().setDismissDelay(200);
    ...
    public double getValue() {
        double value = (double)(cursorX - cursorMin) / (cursorMax - cursorMin)
                * (max - min) + min;
    ...
    public void setValue(double value) {
        cursorX = (int)((cursorMax - cursorMin)
                * ((log ? Math.log(value) : value) - min) / (max - min)) + cursorMin;
        this.repaint();
    }
...
//    }
    ...
    private Image getCursorImage(int width, int height) {
        ...
        RadialGradientPaint paint = new RadialGradientPaint(
                center, radius, focus, dist, colors, CycleMethod.NO_CYCLE);
    ...
    private Image getBackgroundImage(int width, int height, int offset) {
        ...
        Area area = new Area(new RoundRectangle2D.Double(
                offset, 0, width-offset, height, height, height));
        graphics.setPaint(paint);
        graphics.fill(area);
        area.reset();
        ...
        paint = new LinearGradientPaint(start, end,
                dist, colors, CycleMethod.NO_CYCLE);
        area = new Area(new RoundRectangle2D.Double(
                0, 0, width-offset, height, height, height));
        graphics.setPaint(paint);
        graphics.fill(area);
        area.reset();
    ...
    private Image getTickImage(int width, int height) {
        ...
        LinearGradientPaint paint = new LinearGradientPaint(start, end,
                dist, colors, CycleMethod.NO_CYCLE);
        Area area = new Area(new RoundRectangle2D.Double(
                0, 0, width-offset, height, width, width));
        graphics.setPaint(paint);
        graphics.fill(area);
        area.reset();
        ...
        paint = new LinearGradientPaint(start, end,
                dist, colors, CycleMethod.NO_CYCLE);
        area = new Area(new RoundRectangle2D.Double(
                offset, 0, width-offset, height, width, width));
        graphics.setPaint(paint);
        graphics.fill(area);
        area.reset();
