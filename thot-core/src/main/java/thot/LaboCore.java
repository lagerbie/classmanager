...
import thot.vlc.event.MediaPlayerAdapter;
import thot.vlc.event.MediaPlayerEvent;
...
public abstract class LaboCore implements ProjectManager, IndexProcessing {
    ...
    /**
     * Modifie le type de média du module multimédia. Il peut prendre les valeurs suivantes:
     * <code>StudentCore.UNLOAD</code>,
     * <code>StudentCore.AUDIO</code>,
     * <code>StudentCore.VIDEO</code>,
     * <code>StudentCore.IMAGE</code>.
     *
     ...
     *
     * @return <code>true</code> si on est en lecture automatique.
     */
...
//        setRecordTimeMaxByIndexes();
     ...
     *
     * @return l'index le plus proche du temps ou <code>null</code>.
     */
...
//                min = currentIndex.getFinalTime() - time;
     ...
     *
     * @return <code>true</code> si l'on est sur un index d'enregistrement.
     */
...
//            time = -1;//v0.88: pour initialiser VLC en version 1.1.x avec la vidéo
            ...
            if (time <= 0) {
                ...
                while (time <= 0 || !mediaPlayer.isPlaying()) {
                    ...
                    if (mediaPlayer.isEndReached() || mediaPlayer.getState() > 5) {
