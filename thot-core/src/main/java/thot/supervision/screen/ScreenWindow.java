...
public class ScreenWindow extends ProgressThread implements Runnable {
     ...
     * Instance de log.
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(ScreenWindow.class);
    ...
    /**
     * Socket pour l'envoi des contrôles de la souris et du clavier en prise en main.
     */
    private Socket socketEnvoi;
    /**
     * OutputStream pour l'envoi des contrôles de la souris et du clavier en prise en main.
     */
    ...
    /**
     * Démarre la réception de l'écran. Equivalent à {@code start(addressIP, port, priseenmain, null, 0)}.
     *
     ...
     * @param control {@code true} pour une prise en main.
    ...
    /**
     * Démarre la réception de l'écran. Equivalent à {@code start(addressIP, port, priseenmain, nom, numero, 0, 0,
     ...
     */
    public void start(String addressIP, int port, boolean control, int nbLines, String nom) {
     ...
     * @param nom le nom du diffuseur.
     */
    private void start(String addressIP, int port, boolean control, int nbLines, String nom, int x, int y, int width,
            int height) {
...
//        showWindow(false);
    ...
    private boolean connectReception() {
        ...
        try {
            ...
            LOGGER.debug("...ScreenWindow.connectReception success");
            inputStream = new BufferedInputStream(socketReception.getInputStream());
        } catch (IOException e) {
            LOGGER.warn("Error in ScreenWindow.connectReception: " + e);
            connected = false;
        }
        return connected;
    }
    ...
    private void disconnectReception() {
        if (socketReception != null) {
            try {
                ...
                LOGGER.error("Error in ScreenWindow.disconnectReception", e);
            }
        }
    }
    ...
    /**
     * Connection de la socket pour l'envoi des contrôle lors de la prise en main.
     */
    private void connectEnvoi() {
        ...
        try {
            ...
            LOGGER.error("Error in ScreenWindow.connectEnvoi: ", e);
        }
    }
    ...
    private void disconnectEnvoi() {
        if (socketEnvoi != null) {
            try {
                ...
                LOGGER.error("Error in ScreenWindow.disconnectEnvoi: ", e);
            }
        }
    }
    ...
    private void send(byte[] buffer) {
        if (control && run) {
            ...
            try {
                ...
                LOGGER.error("Error in ScreenWindow.send: ", e);
            }
        }
    }
...
//        } else
     ...
     *
     * @return la manière de sortir ({@code  <0 } si erreur de connexion).
     */
        ...
        try {
            ...
            while (header >= 0) {
                ...
                if (tracker.getErrorsID(0) == null) {
                    ...
                    LOGGER.warn("err in track=" + tracker.getErrorsID(0));
                    LOGGER.warn("remoteTemp[position]:" + remoteTemp[position]);
                    Object[] errors = tracker.getErrorsAny();
                    for (Object object : errors) {
                        LOGGER.warn("err object in track: " + object);
                    }
                }
            ...
            LOGGER.debug("ScreenWindow.analyseJpeg timeout: " + timeout);
            end = -1;
        } catch (IOException e) {
            LOGGER.error("IO error in ScreenWindow.analyseJpeg: ", e);
            end = -2;
        } catch (Exception e) {
            //erreur non standard comme ArrayIndexOutOfBoundsException; au cas où
            LOGGER.error("Error in ScreenWindow.analyseJpeg: ", e);
            end = -3;
        }
    ...
    public void run() {
            ...
            if (!isReceptionConnected()) {
                try {
                    if (connectReception()) {
                        ...
                        if (errorConnected > 32) {
                            ...
                            LOGGER.error("ScreenWindow error limit in connect");
                        } else {
                            Utilities.waitInMillisecond(100);
                        }
                    }
                } catch (Exception e) {
                    LOGGER.error("ScreenWindow error in connect", e);
                    run = false;
                }
            } else {
                ...
                if (analyse < -1) {
                    ...
                    if (error > 32) {
                        ...
                        LOGGER.error("ScreenWindow error limit in analyseJpeg");
                    }
                } else {
                    if (analyse < 0) {
                        // timeout atteint
        ...
        LOGGER.info("ScreenWindow global: " + global / secondToNano
                + " analyse: " + analyseTime / secondToNano
                + " transfert: " + transfertTime / secondToNano
                + " passe: " + passe);
        if (passe > 0) {
            LOGGER.info("ScreenWindow average global: "
