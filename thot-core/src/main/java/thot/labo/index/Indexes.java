...
//    }
    ...
    private void removeIndexesIn(long beginTime, long endTime) {
        ...
        for (Index index : indexes) {
            if (index.getInitialTime() >= beginTime && index.getFinalTime() <= endTime) {
                //supp de l'index compris dans l'intervale sans égalé l'intervale
                if (index.getInitialTime() != beginTime || index.getFinalTime() != endTime) {
                    removeIndexes.add(index);
                }
            }
    ...
    private void updateIndexes(Index newIndex, Index oldIndex) {
        if ((newIndex == null && oldIndex == null) || (newIndex != null && oldIndex != null)) {
            return;
        }
        ...
        if (oldIndex == null && newIndex.isTimeLineModifier()) {
            ...
            if (newIndex.getRate() != Index.NORMAL_RATE) {
                timeOffset = (long) (newIndex.getLength() * (1 - newIndex.getRate()) / newIndex.getRate());
            }
        }
     ...
     *
     * @return {@code true}si l'index à une durée nulle.
     */
     ...
     * @param currentIndex l'index.
     *
     * @return {@code true}si l'index est à cheval avec un autre index.
     */
    ...
    private boolean checkValidity(Index currentIndex) {
        return !(isNullSize(currentIndex) || isOverlapped(currentIndex) || isIdentical(currentIndex));
    }
