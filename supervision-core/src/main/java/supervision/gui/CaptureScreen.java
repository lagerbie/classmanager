...
public class CaptureScreen extends ProgressThread implements Runnable {
     ...
     */
    private MouseKeyboardControl mouseKeyboardControl = null;
    /**
     * Image du curseur.
     */
    ...
    /**
     * Robot pour créer la capture d'écran et la gestion de la souris en mode
     * annoter.
     */
    private Robot robot;
    ...
    private void initSockets(int portBase, int nbClient) {
        ...
        for (int i = 0; i < nbClient; i++) {
            try {
                int port = portBase + i;
                CommonLogger.debug("CaptureScreen ecoute port: " + port);
                ...
                CommonLogger.info("CaptureScreen connecté au port: " + port
                        + " adresse: " + sockets[i].getRemoteSocketAddress());
            } catch (IOException e) {
                CommonLogger.error("CaptureScreen IOException au client: " + i);
                CommonLogger.error(e);
            } catch (Exception e) {
                CommonLogger.error("CaptureScreen Exception au client: " + i);
                CommonLogger.error(e);
            }
        }
    }
    ...
    public void start(int portBase, int nbClient, boolean control) {
        ...
        CommonLogger.info("CaptureScreen start with fps: " + fps + " quality: "
                + quality + " nbLines: " + nbLines);
        run = true;
        ...
        while (run) {
            ...
            while (line < nbLines) {
                ...
                try {
                    ...
                    if (!converted) {
                        CommonLogger.warning("CaptureScreen ImageIO.write return false");
                        continue;
                    }
//                    bufferedImage.flush();
                } catch (Exception e) {
                ...
                if (checksumSave[line] != checksum) {
                    ...
                    while (client < nbClient) {
                        ...
                        try {
                            ...
                            CommonLogger.error("CaptureScreen IOException: "
                                    + e.getMessage() + " for client: " + client
                                    + " ip: " + sockets[client].getInetAddress());
                            CommonLogger.error(e);
...
//                graphics.dispose();
        ...
        CommonLogger.info("global: " + global / secondToNano
                ...
                + " passe: " + passe);
        if (passe > 0) {
            CommonLogger.info("average global: " + global / secondToNano / passe
